...
In practice it is impossible compute the set of all candidate SCPs when the set of allowable cognitive operations $M$ in a planning task is $\Omega^*$ or even $\Omega$. Without a length constraint Proof~\ref{proof:infiniteSCPLength} shows that it is not even possible to compute all possible SCPs for many SCP tasks with a finite set of allowable operations. Table~\ref{tbl:scpSpace} summarises the properties of solution space under different SCP task conditions.
...
$\forall_{\alpha = (\text{v},\text{value}\in \text{domain}(v))} \bar{p}[\textrm{`V'}]:=\bar{p}[\textrm{`V'}] - \alpha$\;
...
A \textit{pseudo-cognitive operation} is a mathematical structure which meet the mathematical definition of a cognitive operations, but does not necessarily have empirical support for its existence in human cognition. The operation $\texttt{th}$ which outputs an epistemic state containing the deductive closure of its input states is one example of a pseudo-cognitive operation. 
