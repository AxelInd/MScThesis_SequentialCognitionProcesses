...
A realised SCP $r=(k=K[\pi],f())$ is said to model the classical inference, if $f(k) = f(s_i \in \pi \longmapsto \texttt{th})$. An SCP $\pi$ is said to model the classical inference if some realised SCP generated from $\pi$ models the classical inference.
