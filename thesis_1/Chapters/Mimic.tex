...
\caption{Realised SCPs for the SCP interpretation of the WST where $\mu=(\pi,f_\text{WST})$, and $\mu=(\pi,f_\text{WST}^\text{pref})$, with $\pi=(s_i \longmapsto \texttt{addAB} \longmapsto \texttt{addExp}  \longmapsto \texttt{wc} \longmapsto \texttt{semantic})$. Results mimic those observed in \cite{holldobler2015weak}'s WST interpretation.}
\label{fig:rSCP_WST}
\end{figure}
