...
Which represents the  task without the suppressing conditional $(l|o)$. Interpreting the conditional as a license for implication, as in Table~\ref{tbl:cond}, the knowledge base can be rewritten as a propositional knowledge base:
...
Because this thesis has used a de finetti truth table to interpret conditionals (rather than converting the conditional into a license for implication and then evluating it using the $\L @TODO$ logic) evaluating the conditional $I_{\{7,D'\}\{3 \text{ab1},\text{ab2},D\}}( 3 | D )$ results in non-applicability. This author suggests that instead of evaluating the condtional $(3|D)$ to determine it is false, a card should be turned in the contrapositional case if $I_{\text{lm wc KB'}\cup \epsilon}(D' | 7)$ does not result in non-applicability. Under this revised interpretation, the cards $D$ and $3$ and no longer turned, but the card $7$ is.

 
\subsubsection*{Combined Model}
By combining our contraposition and adductive rules for inferences which were discussed in \citep{dietz2012computational}'s paper, it is possible to model the case where the cards $D$, $3$, and $7$ are turned. This can be done simply by turning a card if it would be turned in any of the three other cases.

For those who find combining results from multiple models distasteful, it is also possible to model this case modifying the conditional to be evaluated in the contraposition case. By changing the requirement that conditional $I(D' | 7)\neq \text{non-applicability}$ to the requirement that either $I((3|D) \neq \text{non-applicability}$ or $I(D' | 7))\neq \text{non-applicability}$, every card except $K$ will be turned.
